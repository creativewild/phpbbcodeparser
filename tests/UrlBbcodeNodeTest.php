...
class UrlBbcodeNodeTest extends PhpUnit_Framework_TestCase
{
	...
	{
		$this->assertEquals(
			'[url]http://my.super.website.com/path/to/resource.htm[/url]',
			$this->_node->toString()
		);
	}
	...
	{
		$this->assertEquals(
			'<a href="http://my.super.website.com/path/to/resource.htm">http://my.super.website.com/path/to/resource.htm</a>',
			$this->_node->toHtml()
		);
	}
	...
	public function test_simpleParsing()
	{
		...
		$img = $parser->parse("[url]http://my.super.website.com/path/to/resource.htm[/url]");
